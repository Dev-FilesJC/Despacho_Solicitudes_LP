...
if st.session_state.page == "cajas":
    ...
    with st.form(f"salida_cajas_form_{st.session_state.caja_form_version}"):
        ...
        with a:
            ...
            agencia_caja = st.text_input("AGENCIA")
    ...
    if guardar_salida:
        ...
        if not solicitante.strip() or not agencia_caja.strip() or int(cajas_solicitadas) <= 0:
            st.error("Completa SOLICITANTE, AGENCIA y una cantidad de cajas mayor a cero.")
    ...
    if cajas_df.empty:
        ...
        tabla_cajas = cajas_df.drop(columns=["_ID_CAJA_"], errors="ignore").copy()
        st.dataframe(tabla_cajas, use_container_width=True, hide_index=True, height=420, column_config={"FECHA DE SALIDA": st.column_config.DateColumn("FECHA DE SALIDA", format="DD/MM/YYYY", width=130)})
