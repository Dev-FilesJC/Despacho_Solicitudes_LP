...
ss("forzar_actualizacion_alertas", False)
...
def asegurar_datos_pagina():
    ...
    try:
        if pagina == "solicitudes":
            # Mantener las solicitudes y las alertas actualizadas sin recargar
            # toda la aplicación. La primera entrada carga los datos y luego
            # se refrescan como máximo cada 60 segundos.
            ahora_refresco = time.time()
            ultimo_refresco = st.session_state.get("_ultimo_refresco_solicitudes", 0)
            if (
                st.session_state.rows is None
                or st.session_state.get("forzar_actualizacion_alertas", False)
                or ahora_refresco - ultimo_refresco >= 60
            ):
                st.session_state.rows = cargar_solicitudes_supabase()
                st.session_state._ultimo_refresco_solicitudes = ahora_refresco
                st.session_state._ultimo_refresco_alertas = ahora_refresco
                st.session_state.forzar_actualizacion_alertas = False
        elif pagina == "rutas":
            ...
            try:
                ...
                if editing is not None:
                    ...
                    if not repetidos.empty and int(repetidos.iloc[0]["_ID_"]) != registro_id:
                        ...
                        st.session_state.rows = cargar_solicitudes_supabase()
                        st.session_state._ultimo_refresco_solicitudes = time.time()
                        st.session_state.forzar_actualizacion_alertas = True
                        liberar_bloqueo_edicion()
                        st.session_state.editing = None
                        st.session_state.editing_key = ""
...
# =========================================================
# Si otro usuario registró/modificó una solicitud, actualizamos la base
# de solicitudes como máximo cada 60 segundos. Así las alertas no dependen
# de que el usuario cambie de módulo o pulse ACTUALIZAR.
_ahora_alertas = time.time()
_ultimo_alertas = st.session_state.get("_ultimo_refresco_alertas", 0)
if _ahora_alertas - _ultimo_alertas >= 60:
    try:
        st.session_state.rows = cargar_solicitudes_supabase()
        st.session_state._ultimo_refresco_solicitudes = _ahora_alertas
        st.session_state._ultimo_refresco_alertas = _ahora_alertas
        df = st.session_state.rows
    except Exception as e:
        st.warning(f"⚠️ No se pudo actualizar automáticamente las alertas: {e}")
