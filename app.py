...
if st.session_state.page == "comisiones":
    ...
    # El formulario inicia CERRADO y solo se abre cuando el administrador lo solicita.
    # Al cancelar o guardar se vuelve a cerrar automáticamente.
    if not st.session_state.get("mostrar_form_comision", False):
        if st.button("➕ REGISTRAR SALIDA A COMISIÓN", key="abrir_form_comision", use_container_width=True):
            st.session_state.mostrar_form_comision = True
            st.rerun()
    ...
    with st.expander(
        ...
        with st.form(f"form_comision_{st.session_state.comision_form_version}", clear_on_submit=False):
            ...
            # Las horas son opcionales. Solo calculamos el total cuando ambas fueron registradas.
            total_horas = None
    ...
    try:
        ...
        if df_com.empty:
            ...
            vista_com["HORA DE INGRESO"] = vista_com["HORA DE INGRESO"].astype(str).str[:5]
            vista_com["HORA DE SALIDA"] = vista_com["HORA DE SALIDA"].astype(str).str[:5]
            vista_com["TOTAL HORAS"] = pd.to_numeric(vista_com["TOTAL HORAS"], errors="coerce").fillna(0).map(lambda x: f"{x:.2f}")
            st.dataframe(vista_com[COMISION_COLS], use_container_width=True, hide_index=True)
            st.download_button(
