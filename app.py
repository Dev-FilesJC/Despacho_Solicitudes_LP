...
def excel_bytes_informe_cajas_cintillos(df_salidas, df_movimientos=None, titulo_filtro="", fecha_desde=None, fecha_hasta=None):
    """Genera un informe tipo KARDEX, inspirado en el formato del Excel de referencia.
    Incluye trazabilidad de salidas de cajas/cintillos y un resumen ejecutivo.
    """
    ...
    # Cabecera similar al kardex de referencia.
    ...
    # Si se entrega un mes, el informe muestra el mes completo
    # (primer y último día), aunque no existan movimientos todos los días.
    if fecha_desde is not None and fecha_hasta is not None:
        ...
        if len(fechas):
            ...
            hoy = fecha_local_hoy().strftime("%d/%m/%Y")
            desde = hasta = hoy
    ...
    # Convertimos cada salida en movimientos tipo CAJA/CINTILLO.
    filas = []
    if df_salidas is not None and not df_salidas.empty:
        for _, r in df_salidas.iterrows():
            ...
            cintillos = entero_seguro(r.get("CANTIDAD DE CINTILLOS"))

            if cajas:
                filas.append({
                    "codigo": "23001001",
                    "articulo": "CAJA RESGUARDO",
                    "centro": centro,
                    "fecha": fecha,
                    "wo": wo,
                    "cantidad": cajas,
    ...
    for fila_num, r in enumerate(filas, 8):
        ...
        ws.cell(fila_num, 7).number_format = "DD/MM/YYYY"

    # Si no hay datos, dejamos una fila informativa.
    if not filas:
        ws.cell(8, 1, "SIN REGISTROS")
        ws.merge_cells("A8:W8")
        ws["A8"].alignment = Alignment(horizontal="center")
        ws["A8"].font = Font(name="Arial", size=10, italic=True)

    anchos = [14, 34, 15, 36, 14, 14, 14, 16, 13, 13, 13, 12, 12, 13, 12, 18, 15, 22, 16, 16, 14, 14, 16]
    ...
    total_cajas = int(pd.to_numeric(df_salidas.get("CANTIDAD DE CAJAS SOLICITADAS", 0), errors="coerce").fillna(0).sum()) if df_salidas is not None and not df_salidas.empty else 0
    total_cintillos = int(pd.to_numeric(df_salidas.get("CANTIDAD DE CINTILLOS", 0), errors="coerce").fillna(0).sum()) if df_salidas is not None and not df_salidas.empty else 0
    ...
    resumen_data = [
        ...
        ("Total de cintillos solicitados", total_cintillos),
    ...
    for rr, (ind, val) in enumerate(resumen_data, 3):
        ...
        for cc in (1,2):
            ...
            resumen.cell(rr,cc).alignment = Alignment(vertical="center")
    ...
    if es_admin():
        ...
        with b2:
            ...
            st.download_button(
                ...
                disabled=not informe_mensual_disponible,
                help=(
                    "Seleccione un mes en 'FILTRAR POR MES'. "
                    "El archivo se genera con el formato mensual tipo Kardex."
                ),
            )
