            ...
            try:
                ...
                if editing is not None:
                    ...
                    if not repetidos.empty and int(repetidos.iloc[0]["_ID_"]) != registro_id:
                        ...
                        st.session_state.rows = cargar_solicitudes_supabase()
                        st.session_state["buscar_solicitud_live"] = ""
                        st.session_state["filtro_cliente"] = "TODOS"
                        st.session_state["filtro_estado_solicitud"] = "TODOS"
                        st.session_state["filtro_fecha_solicitud"] = None
...
# =========================================================
# Los filtros trabajan sobre el DataFrame que acaba de cargar
# desde Supabase. El texto de búsqueda se conserva entre reruns
# de Streamlit, por eso incluimos un botón para limpiarlo fácilmente.
buscar_col, cliente_col, estado_col, fecha_col, limpiar_col = st.columns([3,1.4,1.4,1.2,0.75])

with buscar_col:
    buscar = st.text_input(
        "Buscar solicitud...",
        placeholder="Cliente, WO, dirección, tipo...",
...
with limpiar_col:
    st.write("")
    if st.button("🧹", help="Limpiar todos los filtros", use_container_width=True, key="limpiar_filtros_solicitudes"):
        st.session_state["buscar_solicitud_live"] = ""
        st.session_state["filtro_cliente"] = "TODOS"
        st.session_state["filtro_estado_solicitud"] = "TODOS"
        st.session_state["filtro_fecha_solicitud"] = None
...
# Búsqueda masiva: coincidencia exacta por NRO SOLICITUD - WO.
if st.session_state.busqueda_masiva_ids:
    ids_masivos = set(st.session_state.busqueda_masiva_ids)
    vista = vista[
        vista["NRO SOLICITUD - WO"].map(normalizar).isin(ids_masivos)
    ]

# Búsqueda libre: ignora mayúsculas/minúsculas y espacios.
# Se limita a las columnas reales de solicitudes para evitar efectos
# inesperados por columnas auxiliares.
...
if buscar_normalizado:
    ...
    mascara_busqueda = pd.Series(False, index=vista.index)
    for columna in columnas_busqueda:
        mascara_busqueda = mascara_busqueda | vista[columna].map(
            lambda x: buscar_normalizado in normalizar(x)
        )
    vista = vista[mascara_busqueda]

# Filtro por cliente.
if filtro_cliente != "TODOS" and "CLIENTE" in vista.columns:
    vista = vista[
        vista["CLIENTE"].map(normalizar).eq(normalizar(filtro_cliente))
    ]

# Filtro por estado.
if filtro_estado != "TODOS" and "ESTADO DE SOLICITUD" in vista.columns:
    vista = vista[
        vista["ESTADO DE SOLICITUD"].map(normalizar).eq(normalizar(filtro_estado))
    ]

# Filtro por fecha.
if filtro_fecha and "FECHA DE INGRESO" in vista.columns:
    fechas = vista["FECHA DE INGRESO"].apply(convertir_fecha)
    vista = vista[fechas.eq(filtro_fecha)]
