...
def cargar_catalogo(categoria, incluir_placeholder=True):
    """Carga un catálogo activo desde Supabase, ordenado alfabéticamente."""
    response = (
...
def agregar_catalogo(categoria, valor):
    ...
    if existente.data:
        return db().table("catalogos_app").update({"activo": True}).eq("id", existente.data[0]["id"]).execute()
    return db().table("catalogos_app").insert({"categoria": categoria, "valor": valor, "activo": True}).execute()


def desactivar_catalogo(categoria, valor):
    """Desactiva un valor sin borrar las solicitudes/registros históricos."""
    return db().table("catalogos_app").update({"activo": False}).eq("categoria", categoria).eq("valor", valor).execute()
...
try:
    ...
    st.info("Verifica que exista la tabla `catalogos_app` y ejecuta el SQL de catálogos que acompaña esta versión.")
    st.stop()
...
if st.session_state.page == "catalogos":
    ...
    st.caption("Los cambios se guardan directamente en Supabase. Desactivar un valor no modifica los registros históricos.")
    ...
    categorias = {
        ...
        "AGENCIAS (CAJAS Y CINTILLOS)": "AGENCIA",
        "OPERADORES (COMISIÓN)": "OPERADOR_COMISION",
        "TIPOS DE SALIDA (COMISIÓN)": "TIPO_SALIDA_COMISION",
    }
    nombre_cat = st.selectbox("Catálogo", list(categorias.keys()), key="admin_catalogo_categoria")
    categoria = categorias[nombre_cat]
    valores_activos = cargar_catalogo(categoria, incluir_placeholder=False)

    a1, a2 = st.columns([3, 1])
    with a1:
        nuevo_valor = st.text_input("➕ Nuevo valor", placeholder="Escribe el nuevo elemento...")
    with a2:
        st.write("")
        st.write("")
    ...
    if agregar:
        try:
            ...
            st.success(f"✅ {nombre_cat[:-1] if nombre_cat.endswith('S') else nombre_cat}: agregado correctamente.")
            st.rerun()
        except Exception as e:
            st.error(f"❌ No se pudo agregar: {e}")

    st.markdown("### 🗑️ Desactivar valor")
    if valores_activos:
        valor_eliminar = st.selectbox("Selecciona el valor que deseas desactivar", valores_activos, key="admin_catalogo_eliminar")
        if st.button("🗑️ DESACTIVAR", type="primary", use_container_width=True):
            try:
    ...
    st.markdown(f"**Valores activos: {len(valores_activos)}**")
    if valores_activos:
        st.dataframe(pd.DataFrame({"VALOR": valores_activos}), use_container_width=True, hide_index=True, height=300)

    if st.button("⬅ VOLVER A SOLICITUDES", key="volver_catalogos"):
        st.session_state.page = "solicitudes"
        st.rerun()
...
if st.session_state.page == "comisiones":
    ...
    try:
        ...
        if df_com.empty:
            ...
            with f2:
                if filtro_tipo == "OPERADOR":
                    operadores_filtro = [x for x in CAT_OPERADORES_COMISION if x != CAT_OPERADORES_COMISION[0]]
                    ...
                    tipos_filtro = [x for x in CAT_TIPOS_SALIDA_COMISION if x != CAT_TIPOS_SALIDA_COMISION[0]]
                ...
                if not registro_editar.empty:
                    ...
                    with st.expander("✏️ EDITAR REGISTRO SELECCIONADO", expanded=True):
                        with st.form(f"form_editar_comision_{id_editar}_{st.session_state.comision_tabla_version}", clear_on_submit=False):
                            ...
                            with e2:
                                tipo_actual = str(reg.get("TIPO DE SALIDA", ""))
                                tipo_edit = st.selectbox("🚶 TIPO DE SALIDA", CAT_TIPOS_SALIDA_COMISION, index=CAT_TIPOS_SALIDA_COMISION.index(tipo_actual) if tipo_actual in CAT_TIPOS_SALIDA_COMISION else 0)
