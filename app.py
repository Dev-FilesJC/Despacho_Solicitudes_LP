...
if st.session_state.page == "cajas":
    ...
    if es_admin():
        with st.expander("➕ INGRESAR LLEGADA DE STOCK", expanded=False):
            st.caption("🔴 Solo ADMIN puede aumentar o ajustar el stock.")
            with st.form(f"stock_form_{st.session_state.stock_form_version}"):
                a,b = st.columns(2)
                with a:
                    fecha_ingreso_stock = st.date_input("📅 Fecha de llegada", value=fecha_local_hoy(), format="DD/MM/YYYY")
                    cajas_nuevas_in = st.number_input("📦 Cajas nuevas recibidas", min_value=0, step=1, value=0)
                    cajas_reutilizadas_in = st.number_input("♻️ Cajas reutilizadas ingresadas", min_value=0, step=1, value=0)
                with b:
                    cintillos_in = st.number_input("🏷️ Cintillos recibidos", min_value=0, step=1, value=0)
                    proveedor = st.text_input("🚚 Proveedor / procedencia")
                    obs_ingreso = st.text_area("📝 Observaciones")
                guardar_stock = st.form_submit_button("💾 GUARDAR INGRESO DE STOCK", use_container_width=True)
            if guardar_stock:
                if int(cajas_nuevas_in)+int(cajas_reutilizadas_in)+int(cintillos_in) <= 0:
                    st.error("Ingresa al menos una cantidad mayor a cero.")
                else:
                    guardar_movimiento_stock(fecha_ingreso_stock, "INGRESO", cajas_nuevas_in, cajas_reutilizadas_in, cintillos_in, proveedor, obs_ingreso)
                    st.session_state.cajas = cargar_cajas_supabase()
                    st.session_state.movimientos_stock = cargar_movimientos_stock_supabase()
    ...
    st.markdown("### 📝 Registrar salida de cajas y cintillos")
    with st.form(f"salida_cajas_form_{st.session_state.caja_form_version}"):
        ...
        with y:
            cancelar_salida = st.form_submit_button("✕ CANCELAR", use_container_width=True)
    ...
    if es_admin():
        ...
        if st.session_state.mostrar_filtro_historial_salidas and not cajas_df.empty:
            ...
            with f1:
                ...
                fechas_validas = fechas_hist.dropna()

                if not fechas_validas.empty:
                    fecha_min = fechas_validas.min().date()
                    fecha_max = fechas_validas.max().date()
                else:
                    fecha_min = fecha_local_hoy()
                    fecha_max = fecha_local_hoy()

                rango_fechas = st.date_input(
                    "📅 RANGO DE FECHAS",
                    value=(fecha_min, fecha_max),
                    min_value=fecha_min,
                    max_value=max(fecha_max, fecha_min),
                    key="filtro_historial_rango_fechas",
                )

            with f2:
                clientes_hist = sorted(
                    [
                        str(x)
                        for x in cajas_df.get("CLIENTE", pd.Series(dtype=str))
                        .dropna()
                        .unique()
                        .tolist()
                        if str(x).strip()
                ...
                filtro_cliente_hist = st.selectbox(
                    "CLIENTE",
                    ["TODOS"] + clientes_hist,
            ...
            with f3:
                agencias_hist = sorted(
                    [
                        str(x)
                        for x in cajas_df.get("AGENCIA", pd.Series(dtype=str))
                        .dropna()
                        .unique()
                        .tolist()
                        if str(x).strip()
                ...
                filtro_agencia_hist = st.selectbox(
                    "AGENCIA",
                    ["TODAS"] + agencias_hist,
                    key="filtro_historial_agencia",
                )

            f4, f5 = st.columns([1.5, 1.5])
            with f4:
                filtro_solicitante_hist = st.text_input(
                    "SOLICITANTE",
                    placeholder="Buscar solicitante...",
                    key="filtro_historial_solicitante",
                )
            with f5:
                filtro_wo_hist = st.text_input(
                    "NRO. WORKORDER",
                    placeholder="Buscar WorkOrder...",
                    key="filtro_historial_workorder",
                )

            if isinstance(rango_fechas, tuple) and len(rango_fechas) == 2:
                fecha_inicio, fecha_fin = rango_fechas
            elif isinstance(rango_fechas, list) and len(rango_fechas) == 2:
                fecha_inicio, fecha_fin = rango_fechas
            else:
                fecha_inicio = fecha_fin = rango_fechas

            fechas_filtradas = pd.to_datetime(
                tabla_filtrada["FECHA DE SALIDA"], errors="coerce"
            ).dt.date
            tabla_filtrada = tabla_filtrada[
                fechas_filtradas.apply(
                    lambda x: x is not None
                    and not pd.isna(x)
                    and fecha_inicio <= x <= fecha_fin
                )
            ]

            if filtro_cliente_hist != "TODOS":
                tabla_filtrada = tabla_filtrada[
                    tabla_filtrada["CLIENTE"].astype(str).str.strip()
                    == filtro_cliente_hist
                ]

            if filtro_agencia_hist != "TODAS":
                tabla_filtrada = tabla_filtrada[
                    tabla_filtrada["AGENCIA"].astype(str).str.strip()
                    == filtro_agencia_hist
                ]

            if filtro_solicitante_hist.strip():
                texto = filtro_solicitante_hist.strip().lower()
                tabla_filtrada = tabla_filtrada[
                    tabla_filtrada["SOLICITANTE"].astype(str).str.lower().str.contains(
                        texto, na=False
                    )
                ]

            if filtro_wo_hist.strip():
                texto = filtro_wo_hist.strip().lower()
                tabla_filtrada = tabla_filtrada[
                    tabla_filtrada["NRO. WORKORDER"].astype(str).str.lower().str.contains(
                        texto, na=False
                    )
                ]
        ...
        with b3:
            if st.button(
                "✏️ EDITAR REGISTROS",
                use_container_width=True,
                key="btn_editar_historial_salidas",
                disabled=tabla_filtrada.empty,
            ):
                st.session_state.mostrar_editor_historial_salidas = (
                    not st.session_state.mostrar_editor_historial_salidas
                )
                st.rerun()

        if st.session_state.mostrar_editor_historial_salidas and not tabla_filtrada.empty:
            ...
            opciones_edicion = tabla_filtrada["_ID_CAJA_"].astype(int).tolist()
            mapa_edicion = {
                int(r["_ID_CAJA_"]): (
                    f"ID {int(r['_ID_CAJA_'])} · {r.get('SOLICITANTE', '')} · "
                    f"{r.get('AGENCIA', '')} · {r.get('FECHA DE SALIDA', '')}"
                )
                for _, r in tabla_filtrada.iterrows()
            }
            salida_id_editar = st.selectbox(
                "Selecciona el registro a editar",
                opciones_edicion,
                format_func=lambda x: mapa_edicion.get(int(x), str(x)),
                key="selector_edicion_historial_salidas",
            )
