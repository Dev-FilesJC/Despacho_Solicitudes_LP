...
COMISION_COLS = ["OPERADOR", "FECHA", "TIPO DE SALIDA", "HORA DE INGRESO", "HORA DE SALIDA", "TOTAL HORAS", "COMPENSADO", "FECHA COMPENSACIÓN", "DESCRIPCIÓN"]
...
def excel_bytes_comisiones(df):
    ...
    if "COMPENSADO" in datos.columns:
        datos["COMPENSADO"] = datos["COMPENSADO"].apply(
    ...
    if "TOTAL HORAS" in datos.columns:
        ...
        ).map(lambda x: round(float(x), 2) if pd.notna(x) else "")
    ...
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        ...
        resumen = pd.DataFrame({
            ...
            "VALOR": [
                ...
                round(horas_compensadas, 2),
                round(horas_pendientes, 2),
            ],
        })
...
if st.session_state.page == "comisiones":
    ...
    try:
        ...
        if df_com.empty:
            ...
            with f2:
                if filtro_tipo == "OPERADOR":
                    ...
                    fecha_filtro = st.date_input(
                        ...
                        value=fecha_local_hoy(),
                        format="DD/MM/YYYY",
                        key="filtro_comisiones_fecha",
                    )
                    fechas = df_filtrado["FECHA"].apply(convertir_fecha)
                    df_filtrado = df_filtrado[
                        fechas.apply(lambda x: x == fecha_filtro if x else False)
                    ]
            ...
            horas_acumuladas = float(horas_serie.sum())
            horas_compensadas = float(horas_serie[compensado_bool].sum())
            horas_pendientes = max(0.0, horas_acumuladas - horas_compensadas)

            st.markdown("### ⏱️ RESUMEN DE HORAS PARA COMPENSACIÓN")
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("📊 REGISTROS FILTRADOS", len(df_filtrado))
            m2.metric("⏱️ HORAS ACUMULADAS", f"{horas_acumuladas:.2f} h")
            m3.metric("✅ HORAS COMPENSADAS", f"{horas_compensadas:.2f} h")
            m4.metric("🕐 HORAS PENDIENTES", f"{horas_pendientes:.2f} h")
            ...
            vista_com["COMPENSADO"] = vista_com["COMPENSADO"].apply(
                lambda x: "✅ SÍ" if es_valor_compensado(x) else "⏳ NO"
            )
                ...
                if not registro_editar.empty:
                    ...
                    with st.expander("✏️ EDITAR REGISTRO SELECCIONADO", expanded=True):
                        with st.form(
                            ...
                            total_edit = calcular_total_horas(
                                fecha_edit,
                                hora_ingreso_edit,
                                hora_salida_edit,
                            )
