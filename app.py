...
REGIONALES = ["Seleccione una opcion", "LA PAZ", "COCHABAMBA", "SANTA CRUZ", "SUCRE", "ORURO", "POTOSI", "TARIJA", "OTRA"]
ZONAS = ["Seleccione una opcion", "ZONA SUR", "ZONA ESTE", "ZONA CENTRO", "ZONA NORTE", "EL ALTO", "LA PAZ", "PROVINCIA", "OTRA"]
...
def formulario_nueva_ruta():
    ...
    with st.form(f"ruta_form_{st.session_state.ruta_form_version}", clear_on_submit=False):
        ...
        with b:
            fecha_se1 = st.date_input("📅 Fecha límite de ingreso (SE1)", value=fecha_local_hoy(), format="DD/MM/YY")
            fecha_sr1 = st.date_input("📅 Fecha límite de ingreso (SR1)", value=fecha_local_hoy(), format="DD/MM/YY")
            fecha_recojo = st.date_input("🚚 Fecha de recojo", value=fecha_local_hoy(), format="DD/MM/YY")
        ...
        with st.expander(
            ...
            with st.form(f"salida_cajas_form_{st.session_state.caja_form_version}"):
                ...
                with a:
                    ...
                    fecha_salida = st.date_input("FECHA DE SALIDA", value=fecha_local_hoy(), format="DD/MM/YY")
            ...
            with st.form(f"editar_salida_form_{int(salida_id_editar)}"):
                ...
                with e1:
                    ...
                    edit_fecha = st.date_input(
                        "FECHA DE SALIDA", value=fecha_edicion, format="DD/MM/YY"
                    )
            ...
            resultado_cajas = st.data_editor(
                ...
                column_config={
                    ...
                    "FECHA DE SALIDA": st.column_config.DateColumn(
                        "FECHA DE SALIDA", format="DD/MM/YY", width=130
                    ),
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "FECHA LIMITE DE INGRESO (SE1)": st.column_config.DateColumn(
                    "FECHA LÍMITE SE1", format="DD/MM/YY", width=150
                ),
                "FECHA LIMITE DE INGRESO (SR1)": st.column_config.DateColumn(
                    "FECHA LÍMITE SR1", format="DD/MM/YY", width=150
                ),
                "FECHA DE RECOJO": st.column_config.DateColumn(
                    "FECHA DE RECOJO", format="DD/MM/YY", width=145
                ),
            },
        )
...
if puede_modificar() and (st.session_state.registro_activo or st.session_state.editing is not None):
    ...
    with st.form(f"solicitud_form_{st.session_state.form_version}"):
        ...
        with b:
            ...
            fecha = st.date_input("📅 Fecha de ingreso", value=fecha_default, format="DD/MM/YY")
        x,y = st.columns([4,1])
    ...
    resultado = st.data_editor(
        ...
        column_config={
            ...
            "FECHA DE INGRESO": st.column_config.DateColumn(
                "FECHA", format="DD/MM/YY", width=105
            ),
