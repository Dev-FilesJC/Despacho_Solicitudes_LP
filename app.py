...
with u2:
    st.markdown("**🗄️ Base de datos:** `Supabase PostgreSQL`")
with u3:
...
if filtro_fecha:
    fechas = pd.to_datetime(vista["FECHA DE INGRESO"], errors="coerce")
...
alertas = df.copy()
fechas_alerta = pd.to_datetime(alertas["FECHA DE INGRESO"], errors="coerce")
alertas["_FECHA_ALERTA"] = fechas_alerta.dt.date
hoy_app = fecha_local_hoy()
alertas["_DIAS_ALERTA"] = (pd.Timestamp(hoy_app) - fechas_alerta.dt.normalize()).dt.days
...
if not alertas.empty:
    ...
    with st.expander("🔽 Ver tabla de alertas de atención", expanded=False):
        if not urgentes.empty:
            ...
            detalle_urg["FECHA DE INGRESO"] = pd.to_datetime(detalle_urg["FECHA DE INGRESO"], errors="coerce").dt.strftime("%d/%m/%Y")
        ...
        if not advertencias.empty:
            ...
            detalle_adv["FECHA DE INGRESO"] = pd.to_datetime(detalle_adv["FECHA DE INGRESO"], errors="coerce").dt.strftime("%d/%m/%Y")
