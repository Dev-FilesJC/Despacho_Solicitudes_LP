...
COMISION_COLS = ["OPERADOR", "FECHA", "TIPO DE SALIDA", "HORA DE INGRESO", "HORA DE SALIDA", "TOTAL HORAS", "DESCRIPCIÓN"]
...
def cargar_comisiones_supabase():
    response = db().table("control_comisiones").select("*").order("fecha", desc=True).order("id", desc=True).execute()
    rows = []
    for r in response.data or []:
        rows.append({
