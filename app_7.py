...
CLIENTES = [
    ...
    "INDUSTRIA Y COMERCIO ALICONSUMO", "EXPLORA BOLIVIA SA",
]
TIPOS = [
...
CENTROS_COSTO = [
    ...
    "137 - BANCO NACIONAL DE BOLIVIA - BENI",	"219 - OBRAJES",	"270 - 12 DE OCTUBRE",	"315 - REMESADORA",	"409 - VIRGEN DEL SOCAVÓN",	"607 - TABLADITA",	"723 - PAMPA DE LA ISLA",	"REGIONAL COCHABAMBA - VINTO",	"REGIONAL EL ALTO - VIACHA",	"REGIONAL LA PAZ - SOPOCACHI",	"REGIONAL SANTA CRUZ - EQUIPETROL",	"REGIONAL SUCRE - 25 DE MAYO", "1057 - EXPLORA BOLIVIA SA",	
...
COMISION_COLS = ["OPERADOR", "FECHA", "TIPO DE SALIDA", "HORA DE INGRESO", "HORA DE SALIDA", "TIEMPO ADICIONAL", "TOTAL HORAS", "COMPENSADO", "FECHA COMPENSACIÓN", "DESCRIPCIÓN"]
TIPOS_SALIDA_COMISION = ["Seleccione una opción", "CONSULTA SIN RETORNO B_SOL", "CONSULTA SIN RETORNO BNB", "RECOJO DE CAJAS REGONALES B_SOL", "RECOJO DE CAJAS REGIIONALES BNB",
                         "SALIDA CON PERMISO", "ENTREGA DE DOCUMENTOS B_FIE", "RECOJO DE CAJAS B_FIE", "SALIDA POR CUMPLEAÑOS", "A CUENTA VACACIONES", "VACACIONES",
                         "CONSULTA MEDICA", "BAJA MEDICA", "FALTA INJUSTIFICADA", "PARO DE TRANSPORTE", "PAPELEOS GESTORA", "HORAS EXTRAS", "REPOCICION DE HORAS"]
OPERADORES_COMISION = ["Seleccione una opción", "Alfredo Riveros", "Jonathan Gonzales", "Luis Alfredo Magne", "Marcos Fernandez", "Marco A. Huaylluco", "Luis Ramiro Llusco",
                       "Edwin Velasco", "Carlos Ortiz Rios", "Jaime Quispe Machaca", "Beymar Josue Sinañi", "Josue Quisbert Cuentas", "Raul Huanca", "Deyvid Conde Estaca",
                       "Juan Carlos Ramos", "Marco Vedia", "Marcelo Mamani Cochi", "Moises Cruz Salcedo"]
...
def cargar_cajas_supabase():
    ...
    response = db().table("control_cajas").select("*").order("fecha_salida", desc=True).order("id", desc=True).execute()
    rows = []
...
# CARGA INICIAL
# =========================================================
if st.session_state.rows is None or st.session_state.rutas is None or st.session_state.cajas is None or st.session_state.movimientos_stock is None or (es_admin() and st.session_state.comisiones is None):
    try:
...
if st.session_state.page == "comisiones":
    ...
    with st.expander(
        ...
        with st.form(
            ...
            with a:
                operador = st.selectbox("👤 OPERADOR", OPERADORES_COMISION)
            ...
            with b:
                tipo_salida = st.selectbox("🚶 TIPO DE SALIDA", TIPOS_SALIDA_COMISION)
            ...
            total_horas = (horas_base + tiempo_adicional) if horas_base is not None else (tiempo_adicional if tiempo_adicional > 0 else None)
            if total_horas is not None:
                st.info(
                    f"⏱️ TOTAL HORAS: {formatear_horas_minutos(total_horas)} "
                    f"(base: {formatear_horas_minutos(horas_base or 0)} + adicional: {formatear_horas_minutos(tiempo_adicional)})"
                )
            else:
                st.caption(
        ...
        if guardar:
            if operador == OPERADORES_COMISION[0]:
                st.error("❌ Selecciona el OPERADOR.")
            elif tipo_salida == TIPOS_SALIDA_COMISION[0]:
                ...
                try:
                    datos = {
                        ...
                        "TIEMPO ADICIONAL": tiempo_adicional,
        ...
        if df_com.empty:
            ...
            with f2:
                if filtro_tipo == "OPERADOR":
                    operadores_filtro = [
                        x for x in OPERADORES_COMISION
                        if x != OPERADORES_COMISION[0]
                    ]
                    ...
                    tipos_filtro = [
                        x for x in TIPOS_SALIDA_COMISION
                        if x != TIPOS_SALIDA_COMISION[0]
                    ]
                    valor_filtro = st.selectbox(
            ...
            m3.metric("✅ HORAS COMPENSADAS", formatear_horas_minutos(horas_compensadas))
            m4.metric("🕐 HORAS PENDIENTES", formatear_horas_minutos(horas_pendientes))
            ...
            vista_com["TIEMPO ADICIONAL"] = vista_com["TIEMPO ADICIONAL"].apply(
                lambda x: "" if pd.isna(pd.to_numeric(x, errors="coerce")) else etiqueta_tiempo_adicional(x)
            )
            ...
            # Selección para editar directamente desde la misma tabla.
            ...
            resultado_com = st.data_editor(
                ...
                disabled=[c for c in tabla_com.columns if c not in ["✏️ EDITAR", "🗑️ ELIMINAR"]],
                column_config={
                    "_ID_COMISION_": None,
                    ...
                    with st.expander("✏️ EDITAR REGISTRO SELECCIONADO", expanded=True):
                        with st.form(
                            ...
                            with e1:
                                ...
                                operadores_ed = OPERADORES_COMISION
                            ...
                            with e2:
                                ...
                                tipo_edit = st.selectbox(
                                    "🚶 TIPO DE SALIDA",
                                    TIPOS_SALIDA_COMISION,
                                    index=TIPOS_SALIDA_COMISION.index(tipo_actual)
                                    if tipo_actual in TIPOS_SALIDA_COMISION else 0,
                                )
                            ...
                            total_edit = (horas_base_edit + tiempo_adicional_edit) if horas_base_edit is not None else (tiempo_adicional_edit if tiempo_adicional_edit > 0 else None)
                            if total_edit is not None:
                                st.info(
                                    f"⏱️ TOTAL HORAS: {formatear_horas_minutos(total_edit)} "
                                    f"(base: {formatear_horas_minutos(horas_base_edit or 0)} + adicional: {formatear_horas_minutos(tiempo_adicional_edit)})"
                                )
                            else:
                        ...
                        if actualizar:
                            if operador_edit == OPERADORES_COMISION[0]:
                                st.error("❌ Selecciona el OPERADOR.")
                            elif tipo_edit == TIPOS_SALIDA_COMISION[0]:
                                st.error("❌ Selecciona el TIPO DE SALIDA.")
                                ...
                                try:
                                    datos_edit = {
                                        ...
                                        "TIEMPO ADICIONAL": tiempo_adicional_edit,
    ...
    with st.expander(titulo_registro, expanded=st.session_state.get("mostrar_form_comision", False)):
        with st.form(f"form_comision_{st.session_state.comision_form_version}", clear_on_submit=False):
            a, b, c = st.columns(3)
            with a:
                operador = st.selectbox("👤 OPERADOR", OPERADORES_COMISION)
                fecha_comision = st.date_input("📅 FECHA", value=fecha_local_hoy(), format="DD/MM/YYYY")
            with b:
                tipo_salida = st.selectbox("🚶 TIPO DE SALIDA", TIPOS_SALIDA_COMISION)
                registrar_hora_ingreso = st.checkbox("Registrar HORA DE INGRESO", value=False)
                hora_ingreso = st.time_input("🕘 HORA DE INGRESO") if registrar_hora_ingreso else None
            with c:
                registrar_hora_salida = st.checkbox("Registrar HORA DE SALIDA", value=False)
                hora_salida = st.time_input("🕘 HORA DE SALIDA") if registrar_hora_salida else None

            descripcion = st.text_area("📝 DESCRIPCIÓN", placeholder="Ej.: Entrega de documentos en agencia...")

            total_horas = None
            if hora_ingreso is not None and hora_salida is not None:
                total_horas = (datetime.combine(fecha_comision, hora_salida) - datetime.combine(fecha_comision, hora_ingreso)).total_seconds() / 3600
                if total_horas < 0:
                    total_horas += 24
                st.info(f"⏱️ TOTAL HORAS CALCULADO: {total_horas:.2f} horas")
            else:
                st.caption("⏱️ TOTAL HORAS: se calculará automáticamente solo cuando registres ambas horas.")

            g, x = st.columns([4, 1])
            with g:
                guardar = st.form_submit_button("💾 GUARDAR SALIDA", use_container_width=True)
            with x:
                cancelar = st.form_submit_button("✕ CANCELAR", type="primary", use_container_width=True)
    ...
    try:
        ...
        if df_com.empty:
            ...
            with f2:
                if filtro_tipo == "OPERADOR":
                    operadores_filtro = [x for x in OPERADORES_COMISION if x != OPERADORES_COMISION[0]]
                    valor_filtro = st.selectbox("Selecciona el operador", ["Todos"] + operadores_filtro, key="filtro_comisiones_operador")
                    if valor_filtro != "Todos":
                        df_filtrado = df_filtrado[df_filtrado["OPERADOR"].astype(str) == valor_filtro]
                elif filtro_tipo == "FECHA":
                    fecha_filtro = st.date_input("Selecciona la fecha", value=fecha_local_hoy(), format="DD/MM/YYYY", key="filtro_comisiones_fecha")
                    fechas = df_filtrado["FECHA"].apply(convertir_fecha)
                    df_filtrado = df_filtrado[fechas.apply(lambda x: x == fecha_filtro if x else False)]
                elif filtro_tipo == "TIPO DE SALIDA":
                    tipos_filtro = [x for x in TIPOS_SALIDA_COMISION if x != TIPOS_SALIDA_COMISION[0]]
                    valor_filtro = st.selectbox("Selecciona el tipo de salida", ["Todos"] + tipos_filtro, key="filtro_comisiones_tipo_valor")
                    if valor_filtro != "Todos":
                        df_filtrado = df_filtrado[df_filtrado["TIPO DE SALIDA"].astype(str) == valor_filtro]
                elif filtro_tipo == "DESCRIPCIÓN":
                    valor_filtro = st.text_input("Buscar en la descripción", placeholder="Escribe una palabra o frase...", key="filtro_comisiones_descripcion")
                    if valor_filtro.strip():
                        df_filtrado = df_filtrado[df_filtrado["DESCRIPCIÓN"].astype(str).str.contains(valor_filtro.strip(), case=False, na=False)]

            if filtro_tipo != "Sin filtro":
                st.caption(f"📊 Registros encontrados: {len(df_filtrado)}")

            vista_com = df_filtrado.copy()
            vista_com["FECHA"] = vista_com["FECHA"].apply(lambda x: convertir_fecha(x).strftime("%d/%m/%Y") if convertir_fecha(x) else "")
            vista_com["HORA DE INGRESO"] = vista_com["HORA DE INGRESO"].astype(str).replace("None", "").str[:5]
            vista_com["HORA DE SALIDA"] = vista_com["HORA DE SALIDA"].astype(str).replace("None", "").str[:5]
            vista_com["TOTAL HORAS"] = pd.to_numeric(vista_com["TOTAL HORAS"], errors="coerce").map(lambda x: f"{x:.2f}" if pd.notna(x) else "")
            ...
            resultado_com = st.data_editor(
                ...
                height=420,
                key=f"tabla_historial_comisiones_{st.session_state.comision_tabla_version}",
                disabled=[c for c in tabla_com.columns if c not in ["✏️ EDITAR"]],
                column_config={
                    "_ID_COMISION_": None,
                    "FECHA": st.column_config.TextColumn("FECHA", width=105),
                    "OPERADOR": st.column_config.TextColumn("OPERADOR", width=150),
                    ...
                    "HORA DE INGRESO": st.column_config.TextColumn("HORA INGRESO", width=110),
                    "HORA DE SALIDA": st.column_config.TextColumn("HORA SALIDA", width=110),
                    "TOTAL HORAS": st.column_config.TextColumn("TOTAL HORAS", width=100),
                    "DESCRIPCIÓN": st.column_config.TextColumn("DESCRIPCIÓN", width=280),
                    "✏️ EDITAR": st.column_config.CheckboxColumn("EDIT.", default=False, width=60),
                },
            )
            ...
            if id_editar is not None:
                ...
                if not registro_editar.empty:
                    ...
                    with st.expander("✏️ EDITAR REGISTRO SELECCIONADO", expanded=True):
                        with st.form(f"form_editar_comision_{id_editar}_{st.session_state.comision_tabla_version}", clear_on_submit=False):
                            e1, e2, e3 = st.columns(3)
                            with e1:
                                op_actual = str(reg.get("OPERADOR", ""))
                                operadores_ed = OPERADORES_COMISION
                                operador_edit = st.selectbox("👤 OPERADOR", operadores_ed, index=operadores_ed.index(op_actual) if op_actual in operadores_ed else 0)
                                fecha_edit = st.date_input("📅 FECHA", value=fecha_edit, format="DD/MM/YYYY")
                            with e2:
                                tipo_actual = str(reg.get("TIPO DE SALIDA", ""))
                                tipo_edit = st.selectbox("🚶 TIPO DE SALIDA", TIPOS_SALIDA_COMISION, index=TIPOS_SALIDA_COMISION.index(tipo_actual) if tipo_actual in TIPOS_SALIDA_COMISION else 0)
                                tiene_ing = st.checkbox("Registrar HORA DE INGRESO", value=hora_ing_edit is not None)
                                hora_ingreso_edit = st.time_input("🕘 HORA DE INGRESO", value=hora_ing_edit or datetime.now().time()) if tiene_ing else None
                            with e3:
                                tiene_sal = st.checkbox("Registrar HORA DE SALIDA", value=hora_sal_edit is not None)
                                hora_salida_edit = st.time_input("🕘 HORA DE SALIDA", value=hora_sal_edit or datetime.now().time()) if tiene_sal else None

                            descripcion_edit = st.text_area("📝 DESCRIPCIÓN", value=str(reg.get("DESCRIPCIÓN", "")))

                            total_edit = None
                            if hora_ingreso_edit is not None and hora_salida_edit is not None:
                                total_edit = (datetime.combine(fecha_edit, hora_salida_edit) - datetime.combine(fecha_edit, hora_ingreso_edit)).total_seconds() / 3600
                                if total_edit < 0:
                                    total_edit += 24
                                st.info(f"⏱️ TOTAL HORAS CALCULADO: {total_edit:.2f} horas")
                            else:
                                st.caption("⏱️ TOTAL HORAS: vacío hasta que se registren ambas horas.")
                            ...
                            with g2:
                                cancelar_ed = st.form_submit_button("✕ CANCELAR", type="primary", use_container_width=True)
            ...
            st.download_button(
                ...
                data=excel_bytes_comisiones(df_com),
                file_name=f"SALIDAS_COMISION_{fecha_local_hoy().isoformat()}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
            )
    except Exception as e:
...
if len(vista):
    ...
    resultado = st.data_editor(
        ...
        disabled=COLS + ["_INDICE_REAL_"] + (
            [] if puede_modificar() else ["✏️ EDITAR", "🗑️ ELIMINAR"]
        ),
        column_config={
            "_INDICE_REAL_": None,
