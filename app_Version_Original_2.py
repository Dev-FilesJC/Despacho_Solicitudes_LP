    ...
    if es_admin():
        ...
        with b2:
            ...
            if informe_mensual_disponible:
                ...
                st.caption(
                    "Los datos se ingresan UNA SOLA VEZ POR CLIENTE y se aplican automáticamente "
                    "a todos sus registros del mes. BASE y TAPA pueden tener precios diferentes."
                )

                editor_clientes = st.data_editor(
                    datos_clientes_editor,
                    hide_index=True,
                    use_container_width=True,
                    num_rows="fixed",
                    column_config={
                        ...
                        "COD. CENTRO COSTO": st.column_config.TextColumn(
                            "COD. CENTRO COSTO",
                            help="Ejemplo: BANCO SOL → 120801. Se aplica a todos los registros de BANCO SOL del mes.",
                        ),
                        "PRECIO BASE": st.column_config.NumberColumn(
                            ...
                            step=0.01,
                            format="%.2f",
                            help="Precio unitario de la BASE 23001001.",
                        ),
                        "PRECIO TAPA": st.column_config.NumberColumn(
                            "PRECIO TAPA",
                            min_value=0.0,
                            step=0.01,
                            format="%.2f",
                            help="Precio unitario de la TAPA 23001002.",
                        ),
                    ...
                    key=f"editor_kardex_clientes_{firma_clientes}",
                )

                editor_clientes["COD. CENTRO COSTO"] = editor_clientes["COD. CENTRO COSTO"].fillna("").astype(str).str.strip()
                editor_clientes["PRECIO BASE"] = pd.to_numeric(editor_clientes["PRECIO BASE"], errors="coerce").fillna(0.0)
                editor_clientes["PRECIO TAPA"] = pd.to_numeric(editor_clientes["PRECIO TAPA"], errors="coerce").fillna(0.0)
                editor_clientes["TIPO DE CAMBIO"] = pd.to_numeric(editor_clientes["TIPO DE CAMBIO"], errors="coerce").fillna(1.0)
