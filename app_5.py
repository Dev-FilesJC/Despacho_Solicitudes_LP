...
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg,#111820 0%,#0b1118 100%) !important;
    border-right: 1px solid #263241 !important;
...
.jc-side-brand span { color:#38bdf8; }
.jc-side-user {
    padding:7px 10px;
    margin-bottom:5px;
...
def cargar_solicitudes_supabase():
    response = (
        ...
        .select("id, cliente, nro_solicitud_wo, tipo_solicitud, centro_costo, prioridad, cantidad_items, estado_solicitud, direccion, fecha_ingreso")
        .order("id", desc=False)
        .execute()
    )
...
if st.session_state.page == "portada":
    st.markdown(
        ...
        <div style="
            ...
            border-radius:16px;
            background:linear-gradient(145deg,#0b1526,#07101d);
            text-align:center;
            box-shadow:0 12px 35px rgba(0,0,0,.25);
        ">
            <div style="font-size:56px;line-height:1;">📦</div>
            <div style="font-size:30px;font-weight:900;color:#f8fafc;margin-top:12px;">
                SISTEMA ALMACÉN LA PAZ - BOLIVIA
            </div>
            <div style="font-size:13px;color:#94a3b8;margin-top:10px;">
                Selecciona un módulo desde la barra lateral para comenzar.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
# =========================================================
# Navegación lateral compacta, inspirada en el diseño mostrado:
# usuario/rol arriba, menú limpio al centro y acciones al final.
MENU_BASE = [
    ("🏠 PORTADA", "portada"),
    ("📁 CONTROL DE SOLICITUDES", "solicitudes"),
    ("🗓️ PROGRAMACIÓN DE RUTAS", "rutas"),
    ("📦 CONTROL DE CINTILLOS", "cajas"),
]

if es_admin():
    MENU_BASE += [
        ("🚶 SALIDAS A COMISIÓN", "comisiones"),
        ("⚙️ ADMINISTRAR CATÁLOGOS", "catalogos"),
    ]

MENU_BASE.append(("➕ NUEVA FUNCIONALIDAD", "proxima_funcionalidad"))

MENU_LABELS = [label for label, _ in MENU_BASE]
MENU_PAGES = {label: page for label, page in MENU_BASE}

pagina_actual = st.session_state.get("page", "solicitudes")
if pagina_actual not in [page for _, page in MENU_BASE]:
    pagina_actual = "portada" if "portada" in MENU_PAGES.values() else "solicitudes"

# Estado inicial de PORTADA sin cambiar el funcionamiento de los módulos.
if pagina_actual == "portada":
    pass

label_actual = next(
    (label for label, page in MENU_BASE if page == pagina_actual),
    MENU_LABELS[0],
)
...
with st.sidebar:
    st.markdown(
        '<div class="jc-side-brand">📦 <span>JC CODE</span> SISTEM</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div class="jc-side-user">👤 <b>Usuario:</b> {st.session_state.usuario_nombre}<br>🔐 <b>Rol:</b> {obtener_rol().upper()}</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="jc-side-section">Navegación</div>',
        unsafe_allow_html=True,
    )

    seleccion_menu = st.selectbox(
        "Seleccione una opción",
        MENU_LABELS,
        index=MENU_LABELS.index(label_actual),
        key="menu_navegacion_principal",
    )

    pagina_seleccionada = MENU_PAGES[seleccion_menu]

    st.markdown(
        '<div class="jc-side-section">Sistema</div>',
        unsafe_allow_html=True,
    )
    ...
    actualizar = st.button(
        ...
        key="actualizar_seccion_principal",
    )

    st.caption("🗄️ PostgreSQL • Supabase")

# Cambiar de módulo desde el menú.
if pagina_seleccionada != st.session_state.page:
    liberar_bloqueo_edicion()
    st.session_state.editing = None
    st.session_state.editing_key = ""
    st.session_state.page = pagina_seleccionada
    st.rerun()

# Actualizar únicamente la sección que está abierta
if actualizar:
    try:
...
# =========================================================
# ESPACIO RESERVADO PARA UNA NUEVA FUNCIONALIDAD
# =========================================================
if st.session_state.page == "proxima_funcionalidad":
    st.subheader("➕ NUEVA FUNCIONALIDAD")
    st.info(
        "Este espacio queda reservado para incorporar el próximo módulo "
        "del sistema sin modificar el menú de navegación existente."
    )
    st.markdown(
        """
        **Módulos actuales**
        - 📁 Control de Solicitudes
        - 🗓️ Programación de Rutas
    ...
    st.success(
        "Cuando definamos la nueva funcionalidad, se puede agregar aquí "
        "y aparecerá automáticamente como una nueva opción del menú."
    )
    st.stop()
